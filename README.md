...
## ✅ What works today (stage-1 reference, 12× adversarially reviewed)
...
- **Paper + RLA hybrid** ([profile](docs/PAPER_RLA_PROFILE.md), [ADR-0009](docs/ADRs/ADR-0009-the-digital-paper-anchor-binds-the-board-root-to-the-paper-bal.md)) — a signed **digital↔paper anchor** binds the bulletin-board root to a paper ballot manifest (so neither record can be swapped undetected), plus a VotingWorks **Arlo/SHANGRLA** manifest export, for the supervised polling-place profile where **paper is the legal record and a risk-limiting audit on the paper** delivers software independence ([ADR-0004](docs/ADRs/ADR-0004-paper-ballots-of-record-plus-risk-limiting-audits-are-the-on.md)). Secret-ballot paths use ballot-**polling** RLAs — *never* per-ballot CVRs (secrecy). *(The digital layer is the transparent audit companion, not the legal record; the RLA-of-record is run by external tools on the paper.)*

Twelve independent adversarial crypto-review rounds; every finding fixed (see [docs/CRYPTO_REVIEW.md](docs/CRYPTO_REVIEW.md)). The rest of this README is the modernized specification and roadmap.
