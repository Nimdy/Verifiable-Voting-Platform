...
## ✅ What works today (stage-1 reference, 19× adversarially reviewed)
...
- **Ranked-choice IRV (instant-runoff)** — full end-to-end verifiable IRV via a **verifiable re-encryption mixnet** (Sako–Kilian cut-and-choose, soundness 2⁻¹²⁸): ballots are shuffled to break the voter↔ballot link, *the shuffle is proven correct*, then the anonymized ballots are threshold-decrypted and tabulated round-by-round — and the verifier re-runs the elimination itself. *(Honest scope: IRV **reveals the anonymized ranking multiset** and hides only **which voter** cast which ranking — weaker than Borda, which never reveals a ballot. The O(N) Terelius–Wikström shuffle upgrade now has an **experimental** reference implementation — see below.)*
- **Terelius–Wikström O(N) shuffle** ([mixnet-tw.ts](reference/src/mixnet-tw.ts), [ADR-0012](docs/ADRs/ADR-0012-terelius-wikstrom-on-shuffle-ships-experimental-not-as-default.md)) — ⚠️ **EXPERIMENTAL, opt-in, NOT the default.** An O(N)-size proof of shuffle (vs the default's O(t·N)): a NUMS-CRS permutation commitment + a product argument (`{g}={e}`) + a both-sides-all-W multi-exponentiation, cross-verified TS↔Python with zero divergence and put through a hard adversarial review that found **no soundness gap** (4 LOW/NIT fixed). *Honest scope: a proof of shuffle is the most soundness-treacherous primitive there is (the 2019 SwissPost/Scytl break was a missing check exactly here), and cross-verification + in-house review is **not an external audit** — so the default mix path stays the by-hand-checkable **Sako–Kilian** cut-and-choose, this is wired into no election, and production should use [Verificatum](https://www.verificatum.org/). Single-instance soundness; not constant-time.*
...
- **Selene verifiable trackers (coercion-mitigation)** ([selene.ts](reference/src/selene.ts), [ADR-0011](docs/ADRs/ADR-0011-selene-verifiable-trackers-are-coercion-mitigation-not-resistance.md)) — after the tally, a **verifiable shuffle + threshold-decrypted `(tracker, vote)`** list is posted in cleartext; each voter privately recovers her **tracker point** (`T = β − x·a`, using only her trapdoor key) and reads **her own vote** off the public board — *transparent individual verifiability with no per-voter ZK*. Under coercion she fabricates a deniable **ephemeral opening** (`a′ = x⁻¹·(β − T′)`) that points the same notification at any palatable row while her published dual-key `pk = x·G` is never repudiated. Held to the two-verifier bar (TS + Python, zero divergence). *Honest scope: coercion-**MITIGATION**, **not** coercion-resistance (strictly weaker than JCJ/Civitas — no fake credentials; the public `(tracker,vote)` multiset leaves the forced-instruction "Italian" attack open). In this PoC the trapdoor key + tracker assignment are in-process, so the mitigation is cryptographically **demonstrated, not operative** — a deployment needs voter-contributed keys over an untappable channel + distributed tracker assignment + an eager coercion-free retrieval window. Integrity is classical, not post-quantum.*
...
Nineteen independent adversarial crypto-review rounds; every finding fixed (including a critical anchor-forge caught pre-ship in round 13, the chain-anchor seam's over-claims tightened in rounds 14–15, a dual-verifier scalar-syntax divergence — latent across *all* transcript kinds — caught alongside the everlasting-privacy primitive in round 16, and the everlasting ballot-validity proofs completed in round 17 — see [docs/CRYPTO_REVIEW.md](docs/CRYPTO_REVIEW.md)). The rest of this README is the modernized specification and roadmap.
