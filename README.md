...
## ✅ What works today (stage-1 reference, 15× adversarially reviewed)

A complete, fuzz-tested (~4,800 trials) reference engine in [`reference/`](reference) — *trust root is cryptography, not a chain*:
...
Fifteen independent adversarial crypto-review rounds; every finding fixed (including a critical anchor-forge caught pre-ship in round 13, and the chain-anchor seam's over-claims tightened in rounds 14–15 — see [docs/CRYPTO_REVIEW.md](docs/CRYPTO_REVIEW.md)). The rest of this README is the modernized specification and roadmap.
